...
from pyspanet import SpaNet, SpaNetClient, SpaNetConfig
...
def usage() -> None:
    ...
    print(f"Usage: {sys.argv[0]} <username> <password> <flag>")
    print("\tusername:\tSpaNet username (required)")
    print("\tpassword:\tSpaNet password (required)")
    print("\t-d, --debug:\tenable debug logs (optional)")


async def dump_data(data: SpaNetConfig) -> None:
    """ output spa data """
    print("******** Testing spa connection and configuration **********")

    async with SpaNetClient(data) as spa:
...
async def login(username: str, password: str) -> None:
    """ login to SpaNet """
    async with SpaNet(username, password) as api:
        sockets = await api.get_sockets()
        await dump_data(sockets[0])
